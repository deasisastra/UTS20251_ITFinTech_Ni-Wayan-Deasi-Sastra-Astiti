...
const handler = async (req, res) => {
  ...
  try {
    ...
    if (!phoneNumber) {
      ...
      if (!session?.user?.phone) return res.status(400).json({ message: "Phone required or user must be authenticated" });
    ...
    User.findOneAndUpdate({ phoneNumber }, { phoneVerified: true }, { new: true }).catch(console.error);
  } catch (err) {
    console.error(err);
